...
def check_convergence(p, delta_c, delta_n, epsilon, x_c, x_n, r):
    ...
    while num_generations < max_generations:
        try:
            ...
            return np.inf, np.inf, np.inf,d # Return infinity for both x_c and x_n if OverflowError is encountered

    if num_generations >= max_generations:
        print(f'OverflowError at r = {r}, num_generations = {num_generations}, x_c = {x_c}, x_n = {x_n}, delta_D = {delta_D},d = {d}')
        return np.inf, np.inf, np.inf,np.inf # Return infinity if max_generations is reached
    return x_c, x_n, delta_D,d

# Calculate the status of the function for every r value
for r in r_values:
    x_c, x_n = 0.2, 0.8  # Reset initial conditions
    x_c_final, x_n_final, delta_D_final,d = check_convergence(p,delta_c,delta_n,epsilon,x_c,x_n,r)
