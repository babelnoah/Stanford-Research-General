...
def calculate_next_generation(x_c_initial, x_n_initial, r, delta_n,delta_c,epsilon):
    x_c_next = x_c_initial + r*((x_c_initial + x_n_initial)/2 - x_c_initial + (delta_n)*((x_c_initial - x_n_initial)/2) + (delta_n)*(epsilon))
    x_n_next = x_n_initial + r*((x_c_initial + x_n_initial)/2 - x_n_initial + (delta_c)*(x_c_initial - x_n_initial) + (delta_c)*(epsilon))
...
def check_convergence(delta_c,delta_n,epsilon,x_c,x_n,r):
    ...
    while num_generations < max_generations:
        try:
            ...
            x_c, x_n = calculate_next_generation(x_c, x_n, r, delta_n,delta_c,epsilon)
            x = np.array([x_c,x_n])
...
for r in r_values:
    ...
    x_c_final, x_n_final = check_convergence(delta_c,delta_n,epsilon,x_c,x_n,r)
